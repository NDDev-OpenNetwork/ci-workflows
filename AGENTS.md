...
| `.agents/skills/` | Authored skills: nine portable (`ci-*`, `github-actions-*`). There is no `nddev-repo-flow` skill; operate this repository from `AGENTS.md` and the validators. `.claude/skills/` is a generated byte mirror — regenerate it through the launcher above, never hand-edit. |
