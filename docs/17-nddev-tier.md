# Enterprise tier — what a paid organization adds

The other four tier docs describe what GitHub *offers* at a given visibility and
plan. This one describes what an organization on a paid plan adds on top, so that
a repository under one stops being configured as if it were on the free plan.

Licence counts, repository inventory and observed spend are account state, not
library facts. They belong to whoever operates the account, never to this
library.
  ...
  **Coverage is tracked in the control plane, not here.** Repositories fall into
  ...
  - **The REST enum has no `rust`, but the product does.** `PATCH` accepts only
    ...
    **generally available for default setup** in October 2025. Verified
    ...
    offer it as available on repositories that do contain Rust. So the gap is this account's API surface
...
jobs:
  publish:
    ...
    uses: NDDev-OpenNetwork/ci-workflows/.github/workflows/release-supply-chain.yml@<full-sha>
```
